        ...
        if (hasGallery) {
          const galleryHtml = p.gallery
            .map((item, idx) => {
              ...
              const featured = idx === 0 ? " work-gallery__item--hero" : "";
              return `
                <figure class="work-gallery__item${featured}">
                  <img src="${item.src}" alt="${caption || name}" loading="${idx < 3 ? "eager" : "lazy"}" />
          ...
          return `
            <article class="work-card work-card--gallery" id="${p.id}">
              <div class="work-card__body work-card__body--full">
                <p class="work-card__cat">${meta}</p>
                <h3 class="work-card__name">${name}</h3>
                <p class="work-card__desc">${desc}</p>
              </div>
              <div class="work-gallery" aria-label="${name}">${galleryHtml}</div>
            </article>
          `;
        }

        return `
          <article class="work-card" id="${p.id}">
